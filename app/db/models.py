...
class Role(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, index=True, comment="ID пользователя")
    name: Mapped[str] = mapped_column(nullable=False, unique=True, comment="Название роли")
...
class User(Base):
    ...
    role_id: Mapped[int] = mapped_column(ForeignKey("roles.id"), comment="Роль пользователя")
    active: Mapped[bool] = mapped_column(server_default="true", comment="Активен ли пользователь")
...
class Company(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, index=True, comment="ID компании")
    ...
    main_user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), comment="ID главного пользователя")
...
class Department(Base):
    ...
    address: Mapped[str] = mapped_column(comment="Адрес")
    description: Mapped[str] = mapped_column(comment="Описание")
    company_id: Mapped[int] = mapped_column(ForeignKey("companies.id"), comment="ID компании")

    company: Mapped["Company"] = relationship(back_populates="departments")
    masters: Mapped[list["Master"]] = relationship(back_populates="departments")


class Master(Base):
    __tablename__ = "masters"

    id: Mapped[int] = mapped_column(primary_key=True, index=True, comment="ID мастера")
    name: Mapped[str] = mapped_column(comment="Имя или псевдоним мастера")
    description: Mapped[str] = mapped_column(comment="Описание")
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), comment="ID пользователя")
    company_id: Mapped[int] = mapped_column(ForeignKey("companies.id"), comment="ID компании")
    department_id: Mapped[int] = mapped_column(ForeignKey("departments.id"), comment="ID подразделения")
